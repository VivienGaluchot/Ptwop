...
public class Command extends JPanel {
	...
	private JList<String> linksInfo;
	...
	public Command(Network net) {
		...
		linksInfo = new JList<String>();
		linksInfo.setSelectionMode(ListSelectionModel.SINGLE_INTERVAL_SELECTION);
		linksInfo.setVisibleRowCount(-1);
		linksInfo.setFocusable(false);
	...
	public void update() {
		...
		if (node != null) {
			...
			String[] infos = new String[links.size()];
			for (int i = 0; i < links.size(); i++) {
				infos[i] = "destinatinon : " + links.get(i).getDestNode().getName() + " - "
						+ links.get(i).getNumberOfElements();
			}
			linksInfo.setListData(infos);
			// routageInfo
